...
def get_gammas(counts_bell, counts_comp):
    ...
    for gamma in counts_bell.keys():
        counts_12 = Counter([''.join(list(map(str, elem))) for elem in counts_bell[gamma]['Clicks']])['00']# CHANGED FROM 01 TO 00 FOR TEST OF SIMPLER CIRCUIT.
        counts_1 = Counter([''.join(list(map(str, elem))) for elem in counts_comp[gamma]['Clicks']])['00']# CHANGED FROM 01 TO 00 FOR TEST OF SIMPLER CIRCUIT
